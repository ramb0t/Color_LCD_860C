...
| 2      | 4    | **Flash target address**, big-endian (where the payload is written) |
...
- **Address** = `0x08004000 + block_index × 0x800`
...
So the firmware is written to flash starting at **`0x08004000`** — i.e. a 16 KiB
bootloader occupies `0x08000000–0x08004000` and the application lives above it.
...
Sanity check — the payload begins with a valid STM32 Cortex-M vector table:

| Vector       | Value        |
|--------------|--------------|
| Initial SP   | `0x20010000` |
| Reset        | `0x08019059` |
| NMI / faults | `0x0801909d` |

Load base `0x08004000`; the reset vector `0x08019059` points ~0x15059 into the
image (STM32/GD32 Cortex-M, consistent with the 860C).
...
3. For each 2 KiB chunk of the firmware `.bin`, at load address
